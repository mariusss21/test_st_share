	...
	if row['document'] in indice:
		...
		if not editar:
			...
			with st.form('Form_edit' + str(index)):
				...
				dic['ordem manutenção'] = st_tags(label=('Ordem de manutenção' + ' (' + str(index) + '):'), text='Pressione enter para adicionar', value=doc['ordem manutenção'].replace(']', '').replace('[','').split())
...
def formulario(linhas):
	...
	with st.form('Form_ins'):
		...
		dic['ordem manutenção'] = st_tags(label='Insira as ordens de manutenção', text='Pressione enter para adicionar')
